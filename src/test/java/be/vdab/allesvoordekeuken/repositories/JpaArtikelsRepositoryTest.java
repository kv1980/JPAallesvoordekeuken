...
public class JpaArtikelsRepositoryTest extends AbstractTransactionalJUnit4SpringContextTests{
	...
	private Artikelgroep artikelgroep, artikelgroep2;
...
/*	@Test
	...
	public void kortingToevoegen() {
		Artikel artikel = repository.read(idVanTestartikelFood()).get();
		artikel.addKorting(new Korting(25,BigDecimal.valueOf(33)));
		manager.flush();
		assertEquals(BigDecimal.valueOf(33).setScale(2),super.jdbcTemplate.queryForObject("select percentage from kortingen where artikelid=? and vanafAantal=25",BigDecimal.class,artikel.getId()));
	}
	
	@Test
	public void eenArtikelWordtCorrectToegevoegdAanEenArtikelgroep() {
		manager.persist(artikelgroep);
		repository.create(foodArtikel);
		manager.flush();
		assertEquals("testFood",super.jdbcTemplate.queryForObject(
				"select naam from artikels where artikelgroepid=?",String.class,artikelgroep.getId()));
	}
	...
	public void eenDocentIsNietMeerToegevoegdAanDeOudeCampusNaVerhuis() {
		...
		manager.flush();
		assertEquals(0,super.countRowsInTableWhere("artikels","artikelgroepid = "+artikelgroep.getId()));
	}
